<template>
  <div class="welcome">
      ...
      <div class="content-panel">
        <div class="content-area">
          <el-card class="voice-print-card" shadow="never">
            ...
            >
              <el-table-column
                :label="$t('voicePrint.name')"
                prop="sourceName"
                align="center"
              ></el-table-column>
              <el-table-column
                :label="$t('voicePrint.description')"
                prop="introduce"
                align="center"
              ></el-table-column>
              <el-table-column
                :label="$t('voicePrint.createTime')"
                prop="createDate"
                align="center"
              ></el-table-column>
              <el-table-column :label="$t('voicePrint.action')" align="center">
                <template slot-scope="scope">
                  <el-button
                    size="mini"
                    type="text"
                    @click="editVoicePrint(scope.row)"
                    >{{ $t("voicePrint.edit") }}</el-button
                  >
                  <el-button
                    size="mini"
                    type="text"
                    @click="deleteVoicePrint(scope.row.id)"
                    >{{ $t("voicePrint.delete") }}</el-button
                  >
                </template>
              </el-table-column>
            </el-table>
    ...
    <!-- Add/Edit Parameters Dialog Box -->
    <voice-print-dialog
      :title="dialogTitle"
...
export default {
  ...
  methods: {
    fetchVoicePrints() {
      this.loading = true;
      Api.agent.getAgentVoicePrintList(this.agentId, ({ data }) => {
        this.loading = false;
        if (data.code === 0) {
    ...
    deleteVoicePrint(id) {
      this.$confirm(
        this.$t("voicePrint.confirmDelete"),
        this.$t("voicePrint.warning"),
        {

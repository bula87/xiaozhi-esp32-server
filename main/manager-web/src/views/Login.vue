        ...
        <div class="login-box" @keyup.enter="login">
          ...
          <div style="padding: 0 30px">
            ...
            <div style="display: flex; align-items: center; margin-top: 20px; width: 100%; gap: 10px;">
              ...
              <div style="width: 150px; height: 40px; border: 1px dashed #5778ff; display: flex; align-items: center; justify-content: center; background: rgba(87,120,255,0.05); border-radius: 4px;">
                <img loading="lazy" v-if="captchaUrl" :src="captchaUrl" alt="验证码" style="width: 100%; height: 100%; cursor: pointer" @click="fetchCaptcha" />
                <span v-else style="color: #5778ff; font-size: 11px; cursor: pointer;" @click="fetchCaptcha">Load Captcha</span>
              </div>
...
<script>
// DIRECT IMPORT TO BYPASS API.JS LOOP
...
export default {
  ...
  methods: {
    ...
    fetchCaptcha() {
      try {
        this.captchaUuid = Math.random().toString(36).substring(2);

        // USES userApi NOW
        userApi.getCaptcha(this.captchaUuid, (res) => {
          if (res.status === 200) {
            if (res.data && res.data.code && res.data.code !== 0) {
              alert("Backend Error generating Captcha: " + res.data.msg);
              return;
            }
            const blob = new Blob([res.data], { type: res.data.type || 'image/png' });
            this.captchaUrl = URL.createObjectURL(blob);
          } else {
            alert("Captcha HTTP Error: " + res.status);
          }
        });
      } catch (e) {
        alert("CRASH IN FETCH CAPTCHA: " + e.message);
      }
    },
    ...
    getUserInfo() {
      ...
      userApi.getUserInfo(({ data }) => {
        if (data.code === 0) {
          ...
          showDanger("Failed to get user information");
        }
      });
    },
    ...
    async login() {
      try {
        ...
        try {
          ...
          showDanger(this.$t('sm2.encryptionFailed') || "Encryption Failed");
          return;
        }
        ...
        alert("CRASH IN LOGIN METHOD: " + e.message);
      }
    },

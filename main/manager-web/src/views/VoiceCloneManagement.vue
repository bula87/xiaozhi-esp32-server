  ...
  <div class="welcome">
    ...
    <div class="operation-bar">
      ...
      <div class="right-operations">
        <el-input
          :placeholder="$t('voiceClone.searchPlaceholder')"
          v-model="searchName"
          class="search-input"
        ...
        <div class="content-area">
          ...
          <el-card class="params-card" shadow="never" v-if="total > 0">
            ...
            >
              ...
              <el-table-column :label="$t('voiceClone.name')" align="center">
                <template #default="{ row }">
                  <el-input
                    ...
                    show-word-limit
                    @blur="onNameBlur(row)"
                    @keyup.enter.native="onNameEnter(row)"
                    ref="nameInput"
                  />
              ...
              <el-table-column
                :label="$t('voiceClone.languages')"
                prop="languages"
                align="center"
              ></el-table-column>
              <el-table-column
                :label="$t('voiceClone.trainStatus')"
                prop="trainStatus"
                align="center"
              >
                <template slot-scope="scope">
                  <div
                    class="status-button"
                    :class="getStatusButtonClass(scope.row)"
                  >
              ...
              <el-table-column
                :label="$t('voiceClone.action')"
                align="center"
                width="230"
              >
                <template slot-scope="scope">
                  <el-button
                    v-if="scope.row.hasVoice"
                    size="mini"
                    type="text"
                    @click="handlePlay(scope.row)"
                  >
                    {{
                      playingRowId === scope.row.id
                        ? $t("voiceClone.stop")
                        : $t("voiceClone.play")
                    }}
                  </el-button>
              ...
              <div class="custom-pagination">
                ...
                >
                  <el-option
                    ...
                    :key="item"
                    :label="$t('voiceClone.itemsPerPage', { items: item })"
                    :value="item"
                  >
                  </el-option>
                </el-select>
                ...
                <span class="total-text">{{
                  $t("voiceClone.totalRecords", { total })
                }}</span>
              </div>
            </div>
          </el-card>
    ...
    <VoiceCloneDialog
      ...
      :voiceCloneData="currentVoiceClone"
...
export default {
  ...
  data() {
    return {
      ...
      currentAudio: null, // Current playing audio object
      playingRowId: null, // Current playing row ID
    };
  },
  created() {
  ...
  methods: {
    ...
    handleViewDetails(row) {
      ...
      // You can add the logic for viewing details here
    },
    ...
    handleClone(row) {
      ...
      try {
        Api.voiceClone.cloneAudio(
          ...
          (res) => {
            try {
              ...
              if (res.code === 0) {
                ...
                // Refresh list after successful cloning
                this.fetchVoiceCloneList();
              } else {
                // Refresh list when cloning fails to get complete error information
                console.log(
                  "API returned error, refresh list to get detailed error information",
                );
                this.$message.error(res.msg || this.$t("message.error"));
                // Refresh list to get the complete error details saved by the backend
                this.fetchVoiceCloneList();
              }
            } catch (error) {
              // Error during response processing, refresh list
              console.error("Error during response processing:", error);
              this.$message.error("Error during response processing");
              this.fetchVoiceCloneList();
            } finally {
          ...
          (error) => {
            // API call failed, refresh list to get the latest status
            console.error("API call failed:", error);
            this.$message.error(
              "Cloning failed, please hover your mouse over the error message to view error details",
            );
            this.fetchVoiceCloneList();
            this.$set(row, "_cloning", false);
          },
        ...
        // Error when calling API, refresh list
        console.error("Error when calling API:", error);
        this.$message.error("Error when calling API");
        this.fetchVoiceCloneList();
        this.$set(row, "_cloning", false);
      }
    },
    ...
    updateRowStatus(row, status, statusCode = null) {
      // Directly modifying object properties in the array may not trigger a view update in Vue
      const index = this.voiceCloneList.findIndex((item) => item.id === row.id);
      const updateData = {
        trainStatus: status,
      };

      // If a status code is provided, also update the status code information
      if (statusCode !== null) {
      ...
      if (index !== -1) {
        ...
        // If index is not found, directly update row object
        row.trainStatus = status;
        if (statusCode !== null) {
          row.statusCode = statusCode;
        }
        // Force entire table re-render
        this.$forceUpdate();
      }
      console.log(
        "Update row status:",
    ...
    // Callback after successful cloning
    handleCloneSuccess() {
      this.fetchVoiceCloneList();
    },
    // Enter edit mode
      ...
      Api.voiceClone.updateName(params, (res) => {
        ...
        if (res.code === 0) {
          this.$message.success(
            this.$t("voiceClone.updateNameSuccess") || "Name update successful",
          );
        } else {
          this.$message.error(
            res.msg ||
              this.$t("voiceClone.updateNameFailed") ||
              "Name update failed",
          );
    ...
    // Name input box: submit on blur
    onNameBlur(row) {
      row.isEdit = false;
      setTimeout(() => {
        this.submitName(row);
      }, 100); // Delay 100ms, avoid the window triggered by enter+blur simultaneously
    },
    // Name input box: submit on enter
    onNameEnter(row) {
      row.isEdit = false;
      this.submitName(row);
    },
    // Play audio
    handlePlay(row) {
      // If the clicked row is the currently playing row, stop playback
      ...
      Api.voiceClone.getAudioId(row.id, (res) => {
        ...
        if (res.code === 0) {
          const uuid = res.data;
          // Use the obtained uuid to play the audio
          ...
          // Clear status when playback ends
          audio.addEventListener("ended", () => {
            this.playingRowId = null;
            this.currentAudio = null;
          });

          // Clear status when playback errors
          ...
          audio.play().catch((err) => {
            ...
            this.$message.error(
              this.$t("voiceClone.playFailed") || "Playback failed",
            );
            this.playingRowId = null;
            this.currentAudio = null;
          });
          ...
          this.$message.error(
            res.msg ||
              this.$t("voiceClone.audioNotExist") ||

        ...
        <div class="content-area">
          <el-card class="params-card" shadow="never">
            ...
            >
              ...
              <el-table-column
                :label="$t('otaManagement.firmwareName')"
                prop="firmwareName"
                align="center"
              ></el-table-column>
              <el-table-column
                :label="$t('otaManagement.firmwareType')"
                prop="type"
                align="center"
              >
                <template slot-scope="scope">
              ...
              <el-table-column
                :label="$t('otaManagement.version')"
                prop="version"
                align="center"
              ></el-table-column>
              <el-table-column
                :label="$t('otaManagement.fileSize')"
                prop="size"
                align="center"
              >
                <template slot-scope="scope">
              ...
              <el-table-column
                :label="$t('otaManagement.remark')"
                prop="remark"
                align="center"
                show-overflow-tooltip
              ></el-table-column>
              <el-table-column
                :label="$t('otaManagement.createTime')"
                prop="createDate"
                align="center"
              >
                <template slot-scope="scope">
              ...
              <el-table-column
                :label="$t('otaManagement.updateTime')"
                prop="updateDate"
                align="center"
              >
                <template slot-scope="scope">
              ...
              >
                <template slot-scope="scope">
                  <el-button
                    size="mini"
                    type="text"
                    @click="downloadFirmware(scope.row)"
                    >{{ $t("otaManagement.download") }}</el-button
                  >
                  <el-button
                    size="mini"
                    type="text"
                    @click="editParam(scope.row)"
                    >{{ $t("otaManagement.edit") }}</el-button
                  >
                  <el-button
                    size="mini"
                    type="text"
                    @click="deleteParam(scope.row)"
                    >{{ $t("otaManagement.delete") }}</el-button
                  >
                </template>
              </el-table-column>
            </el-table>
            ...
            <div class="table_bottom">
              <div class="ctrl_btn">
                ...
                >
                  {{
                    isAllSelected
                      ? $t("otaManagement.deselectAll")
                      : $t("otaManagement.selectAll")
                  }}
                </el-button>
                <el-button
                  size="mini"
                  ...
                  >{{ $t("otaManagement.addNew") }}</el-button
                >
                <el-button
                  size="mini"
                  type="danger"
              ...
              <div class="custom-pagination">
                ...
                >
                  <el-option
                    ...
                    :key="item"
                    :label="$t('otaManagement.itemsPerPage', { items: item })"
                    :value="item"
                  >
                  </el-option>
                </el-select>
                ...
                <span class="total-text">{{
                  $t("otaManagement.totalRecords", { total })
                }}</span>
              </div>
            </div>
          </el-card>
...
export default {
  ...
  methods: {
    ...
    fetchFirmwareList() {
      ...
      Api.ota.getOtaList(params, (res) => {
        ...
        if (res.code === 0) {
          ...
          this.$message.error({
            message:
              res?.data?.msg || this.$t("otaManagement.getFirmwareListFailed"),
            showClose: true,
          });
        }
      });
    ...
    showAddDialog() {
      ...
      this.$nextTick(() => {
        // Reset the form's validation status
    ...
    deleteParam(row) {
      ...
      this.$confirm(
        this.$t("otaManagement.confirmBatchDelete", { paramCount }),
        ...
        .then(() => {
          ...
          Api.ota.deleteOta(ids, (res) => {
            ...
            if (res.code === 0) {
              this.$message.success({
                message: this.$t("otaManagement.batchDeleteSuccess", {
                  paramCount,
                }),
                showClose: true,
              });
    ...
    downloadFirmware(firmware) {
      ...
      Api.ota.getDownloadUrl(firmware.id, (res) => {
        if (res.data.code === 0) {
          ...
          window.open(
            `${window.location.origin}${baseUrl}/otaMag/download/${uuid}`,
          );
        } else {
    ...
    async getFirmwareTypes() {
      try {
        ...
        this.$message.error(
          error.message || this.$t("otaManagement.getFirmwareTypesFailed"),
        );
      }
    },

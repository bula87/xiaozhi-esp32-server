          ...
          <el-card class="params-card" shadow="never">
            <div>
              ...
              >
                ...
                <el-table-column
                  :label="$t('knowledgeBaseManagement.name')"
                  prop="name"
                  align="center"
                >
                ...
                <el-table-column
                  :label="$t('knowledgeBaseManagement.description')"
                  prop="description"
                  align="center"
                ...
                <el-table-column
                  :label="$t('knowledgeBaseManagement.documentCount')"
                  align="center"
                >
                  <template slot-scope="scope">
                ...
                <el-table-column
                  :label="$t('knowledgeBaseManagement.status')"
                  align="center"
                >
                  <template slot-scope="scope">
                    <el-switch
                ...
                <el-table-column
                  :label="$t('knowledgeBaseManagement.createdAt')"
                  prop="createdAt"
                  align="center"
                >
                  <template slot-scope="scope">
                ...
                <el-table-column
                  :label="$t('knowledgeBaseManagement.operation')"
                  align="center"
                >
                  <template slot-scope="scope">
                    <el-button
                    ...
                    <el-button
                      size="mini"
                      type="text"
                      @click="deleteSingleKnowledgeBase(scope.row)"
                    >
            ...
            <div class="table_bottom">
              <div class="ctrl_btn">
                <el-button size="mini" type="primary" @click="toggleSelectAll">
                  {{
                    isAllSelected
                      ? $t("knowledgeBaseManagement.cancelSelectAll")
                      : $t("knowledgeBaseManagement.selectAll")
                  }}
                </el-button>
              ...
              <div class="custom-pagination">
                ...
                >
                  <el-option
                    ...
                    :key="item"
                    :label="`${item}${$t('knowledgeBaseManagement.itemsPerPage')}`"
                    :value="item"
                  >
                  </el-option>
                </el-select>
                ...
                <span class="total-text">{{
                  $t("knowledgeBaseManagement.totalRecords", { total })
                }}</span>
              </div>
            </div>
          </el-card>
...
export default {
  ...
  methods: {
    ...
    fetchKnowledgeBaseList: function () {
      ...
      Api.knowledgeBase.getKnowledgeBaseList(
        ...
        (res) => {
          ...
          // Because knowledgeBase.js directly passed the entire response object
          if (res.data && res.data.code === 0) {
            ...
            this.$message.error({
              message:
                res.data?.msg ||
                this.$t("knowledgeBaseManagement.getKnowledgeBaseListFailed"),
              showClose: true,
            });
          }
        },
    ...
    showViewDialog: function (row) {
      // Redirect to the file upload page, pass the knowledge base ID and name
      this.$router.push({
    ...
    handleSubmit: function (form) {
      ...
      if (form.id) {
        ...
        Api.knowledgeBase.updateKnowledgeBase(
          form.datasetId,
          form,
          (res) => {
            ...
            if (res.data && res.data.code === 0) {
              ...
              this.$message.success(
                this.$t("knowledgeBaseManagement.updateSuccess"),
              );
            } else {
              this.$message.error(
                res.data?.msg ||
                  this.$t("knowledgeBaseManagement.updateFailed"),
              );
            }
          },
          (err) => {
        ...
        // Added - only pass necessary fields, do not pass id
        const createData = {
          name: form.name,
        ...
        Api.knowledgeBase.createKnowledgeBase(
          ...
          (res) => {
            ...
            if (res.data && res.data.code === 0) {
              ...
              this.fetchKnowledgeBaseList();
              this.$message.success(
                this.$t("knowledgeBaseManagement.addSuccess"),
              );
            } else {
              this.$message.error(
                res.data?.msg || this.$t("knowledgeBaseManagement.addFailed"),
              );
            }
          },
          (err) => {
            console.log("Error callback received:", err);
            // Error callback processing backend returned error information
            if (err && err.data) {
              console.log(
                "Backend returned error message:",
                err.data.msg || err.msg,
              ...
              this.$message.error(
                err.data.msg ||
                  err.msg ||
                  this.$t("knowledgeBaseManagement.addFailed"),
              );
            } else {
    ...
    deleteSelectedKnowledgeBase: function () {
      if (this.selectedKnowledgeBase.length === 0) {
        this.$message.warning(
          this.$t("knowledgeBaseManagement.selectKnowledgeBaseFirst"),
        );
        return;
      }

      this.$confirm(
        this.$t("knowledgeBaseManagement.confirmBatchDelete", {
          count: this.selectedKnowledgeBase.length,
        }),
        ...
        .then(() => {
          const ids = this.selectedKnowledgeBase
            .map((item) => item.datasetId)
            .join(",");
          Api.knowledgeBase.deleteKnowledgeBases(
            ids,
            (res) => {
              if (res.data && res.data.code === 0) {
                this.fetchKnowledgeBaseList();
                this.$message.success(
                  this.$t("knowledgeBaseManagement.batchDeleteSuccess", {
                    count: this.selectedKnowledgeBase.length,
                  }),
                );
              } else {
                this.$message.error(
                  res.data?.msg ||
                    this.$t("knowledgeBaseManagement.deleteFailed"),
                );
              }
            },
            (err) => {
      ...
      this.$confirm(
        ...
        .then(() => {
          Api.knowledgeBase.deleteKnowledgeBase(
            ...
            (err) => {
              console.log("Error callback received:", err);
              // Handle error callback for backend returned error information
              if (err && err.data) {
                console.log(
                  "Backend returned error message:",
                  err.data.msg || err.msg,
    ...
    handleStatusChange: function (row) {
      // Only pass the fields that need updating, ensure the id field is included
      const updateForm = {
        id: row.id, // Add id field, the backend needs this field to locate the record
        datasetId: row.datasetId,
        name: row.name,
        description: row.description,
        status: row.status,
      ...
      Api.knowledgeBase.updateKnowledgeBase(
        ...
        (res) => {
          ...
          if (res.data && res.data.code === 0) {
            ...
            this.fetchKnowledgeBaseList();
            this.$message.error(
              res.data?.msg || this.$t("knowledgeBaseManagement.updateFailed"),
            );
          }
        },
        () => {

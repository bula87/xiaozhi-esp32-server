    ...
    <div v-else>
      ...
      <div v-else>
        ...
        <div v-else>
          ...
          >
            {{
              $t("cache.totalCachedResources").replace(
                "{count}",
                cacheData.totalCached,
              )
            }}
          </el-alert>

          <h3 class="resource-count">
            {{
              $t("cache.jsResources").replace("{count}", cacheData.js.length)
            }}
          </h3>
          ...
          <h3 class="resource-count">
            {{
              $t("cache.cssResources").replace("{count}", cacheData.css.length)
            }}
          </h3>
...
export default {
  ...
  methods: {
    async loadCacheData() {
      ...
      try {
        // First check if the Cache API is supported
        if (!("caches" in window)) {
        ...
        if (this.cacheAvailable) {
          ...
          // Output complete cache status in the console

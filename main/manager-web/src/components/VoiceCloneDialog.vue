        ...
        <div class="audio-edit-area">
          ...
          <div class="waveform-container">
            ...
            <div
              ...
              v-if="isSelecting || selectionStart !== null"
              :style="selectionStyle"
            ></div>
            <div class="duration-display">
              {{
                $t("voiceClone.selectedDuration", {
...
export default {
  ...
  computed: {
    selectedDuration() {
      if (
        ...
        const start =
          Math.min(this.selectionStart, this.selectionEnd) * duration;
  ...
  methods: {
    ...
    async handleFileChange(file) {
      ...
      // Go to the second step first, ensure DOM is rendered
      this.currentStep = 2;

      // Wait for the DOM to update before loading audio
      await this.$nextTick();
    ...
    async loadAudio(file) {
      try {
        ...
        this.originalAudioBuffer = await this.audioContext.decodeAudioData(
    ...
    drawWaveform() {
      ...
      this.waveformData.forEach((value, index) => {
        // Normalize and enlarge, use 80% of the height

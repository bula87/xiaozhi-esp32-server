  ...
  >
        ...
        >
          ...
          <div class="session-info">
            ...
            <div class="message-count">
              {{ session.chatCount > 99 ? "99" : session.chatCount }}
            </div>
          </div>
        </div>
        ...
        <div v-if="currentSessionId" class="messages">
          <div v-for="(message, index) in messagesWithTime" :key="message.id">
            ...
            <div
              ...
              class="message-item"
              :class="{
                'user-message': message.chatType === 1,
            ...
            >
              <img
                :src="
                  message.chatType === 1
                    ? getUserAvatar(currentSessionId)
                    : require('@/assets/xiaozhi-logo.png')
                "
                class="avatar"
              />
              <div class="message-content">
                <template
                  v-if="
                    Array.isArray(extractContentFromString(message.content))
                  "
                >
                  <div class="content-wrapper">
                    <div
                      v-for="(item, idx) in extractContentFromString(
                        message.content,
                      )"
                      :key="idx"
                    >
                      ...
                      >
                        ...
                        >
                          <div
                            v-if="isToolResultCollapsed(index, idx)"
                            class="tool-result-collapsed"
                          >
                          ...
                          <span
                            ...
                            @click="toggleToolResult(index, idx)"
                          >
                            <i
                              :class="
                                isToolResultCollapsed(index, idx)
                                  ? 'el-icon-arrow-down'
                                  : 'el-icon-arrow-up'
                              "
                            ></i>
                ...
                <i
                  ...
                  :class="getAudioIconClass(message)"
    ...
    <div v-if="currentSessionId" class="download-buttons">
      <el-button
        ...
        plain
        size="small"
        @click="downloadCurrentSessionWithPrevious"
      >
...
export default {
  ...
  computed: {
    messagesWithTime() {
      ...
      const result = [];
      const TIME_INTERVAL = 60 * 1000; // 1 minute interval (milliseconds)
      ...
      for (let i = 0; i < this.messages.length; i++) {
        ...
        // Check if a time marker needs to be added
  ...
  methods: {
     ...
     * If content is in JSON format (e.g., {"speaker": "unknown speaker", "content": "What time is it."}), extract the content field
     * If content is a regular string, return it directly
     *
    ...
    extractContentFromString(content) {
      ...
      try {
        ...
        // If it is an object and has a content field
      ...
      // If not JSON format or no content field, return the original content
      return content;
    },
    // Toggle the expanded/collapsed state of tool results
    ...
    // Determine if the tool result is collapsed
    isToolResultCollapsed(messageIndex, itemIndex) {
      const key = `${messageIndex}-${itemIndex}`;
      // Default to collapsed (true means collapsed)
      return !this.expandedToolResults[key];
    },
    // Get truncated text (show only the first line)
    ...
    playAudio: debounce(function (message) {
      if (this.playingAudioId === message.audioId) {
        // If currently playing audio, stop playback
    ...
    getUserAvatar(sessionId) {
      ...
      // Calculate mod 5 and add 1 to get a number between 1 and 5
    ...
    // Download this session and the first 20 chat histories
    downloadCurrentSessionWithPrevious() {
      Api.agent.getDownloadUrl(this.agentId, this.currentSessionId, (res) => {
        if (res && res.data && res.data.code === 0 && res.data.data) {
          const uuid = res.data.data;
          window.open(
            `${Api.getServiceUrl()}/agent/chat-history/download/${uuid}/previous`,
            "_blank",
          );
        } else {
          this.$message.error(this.$t("chatHistory.downloadLinkFailed"));

...
# Number of pre-buffered packets, send directly to reduce latency
PRE_BUFFER_COUNT = 5
...
async def sendAudioMessage(
    ...
    # Skip residual audio of old sentences
    ...
    if sentenceType == SentenceType.FIRST:
        # Add subsequent messages of the same sentence to the flow control queue, send immediately in other cases
        if (
...
async def _wait_for_audio_completion(conn: "ConnectionHandler"):
    """
    Wait for the audio queue to be empty and wait for pre-buffered packets to finish playing
    ...
    if hasattr(conn, "audio_rate_controller") and conn.audio_rate_controller:
        ...
        conn.logger.bind(tag=TAG).debug(
            f"Waiting for audio sending to complete, there are still {len(rate_controller.queue)} packets in the queue"
        )
        await rate_controller.queue_empty_event.wait()

        # Waiting for pre-buffered packets to finish playback
        # Send the first N packets directly, add 2 network jitter packets, need to wait additionally for them to finish playing on the client
...
async def _send_to_mqtt_gateway(
    ...
    """
    Send the opus data packet with a 16-byte header to mqtt_gateway
    Args:
        conn: connection object
    ...
    # Send the complete data packet including the header
...
async def sendAudio(
    ...
    """
    Send audio packet, use AudioRateController for precise flow control
    ...
    # Unify conversion to list processing
...
def _get_or_create_rate_controller(
    ...
    if not hasattr(conn, "audio_rate_controller"):
        ...
        # Background send task has stopped, needs to reset
        if (
            not rate_controller.pending_send_task
            or rate_controller.pending_send_task.done()
        ):
            need_reset = True
        # Reset needed when sentence_id changes
        elif (
...
async def _send_audio_with_rate_control(
    ...
    Args:
        ...
        send_delay: Fixed delay (seconds), -1 indicates dynamic flow control
    """
    for packet in audio_list:
        ...
        if flow_control["packet_count"] < PRE_BUFFER_COUNT:
            ...
            # Dynamic flow control mode: only added to the queue, sent by the background loop
            rate_controller.add_audio(packet)


async def _do_send_audio(conn: "ConnectionHandler", opus_packet, flow_control):
    """
    Perform actual audio sending
    """
...
async def send_tts_message(conn: "ConnectionHandler", state, text=None):
    ...
    if state == "stop":
        # Save the current sentence_id for subsequent judgment of whether it is the current turn
        ...
        # Check if it is the current turn
        if current_sentence_id != conn.sentence_id:
            return

        # Stop audio sending loop (called only when the rate controller is initialized)
        if hasattr(conn, "audio_rate_controller") and conn.audio_rate_controller:
...
async def send_stt_message(conn: "ConnectionHandler", text):
    ...
    try:
        ...
        if text.strip().startswith("{") and text.strip().endswith("}"):
            ...
            if isinstance(parsed_data, dict) and "content" in parsed_data:
                ...
                # Save speaker information to the conn object
    ...
    await send_tts_message(conn, "start")
    # After sending the start message, the client state will be speaking, synchronize the server state
    conn.client_is_speaking = True


async def send_display_message(conn: "ConnectionHandler", text):
    """Send pure display message"""

        ...
        try:
            ...
            if not self.model:
                logger.bind(tag=TAG).error(
                    "VOSK model not loaded, unable to perform recognition"
                )
                return "", None
            ...
            logger.bind(tag=TAG).debug(
                f"VOSK voice recognition time-consuming: {time.time() - start_time:.3f}s | Result: {text_result.strip()}"
            )

            return text_result.strip(), artifacts.file_path

        except Exception as e:
            logger.bind(tag=TAG).error(f"VOSK voice recognition failed: {e}")
            return "", None

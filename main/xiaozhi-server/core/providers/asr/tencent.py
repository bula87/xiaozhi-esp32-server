...
class ASRProvider(ASRProviderBase):
        ...
        try:
            ...
            if result:
                logger.bind(tag=TAG).debug(
                    f"Tencent Cloud speech recognition time-consuming: {time.time() - start_time:.3f}s | Result: {result}"
                )
    ...
    def _build_request_body(self, base64_audio: str) -> str:
        ...
        request_map = {
            ...
            "EngSerViceType": "16k_zh",  # Mandarin common
    ...
    def _get_auth_headers(self, request_body: str) -> Tuple[str, str]:
        ...
        try:
            ...
            # Note: header information needs to be arranged in ASCII ascending order, and key and value are converted to lowercase
            # Must include content-type and host headers
            content_type = "application/json; charset=utf-8"
            host = "asr.tencentcloudapi.com"
            action = "SentenceRecognition"  # Interface name

            # Build canonical header information, note order and format
            canonical_headers = (
            ...
            logger.bind(tag=TAG).error(
                f"Failed to generate authentication header: {e}", exc_info=True
            )

...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        # If the configuration is an internal address (containing -internal.aliyuncs.com), use ws protocol, default is wss protocol
        if "-internal." in self.host:
            self.ws_url = f"ws://{self.host}/ws/v1"
        else:
            # Default to using wss protocol
        ...
        if self.access_key_id and self.access_key_secret:
            ...
            raise ValueError(
                "Must provide access_key_id+access_key_secret or directly provide token"
            )
    ...
    async def receive_audio(self, conn, audio, audio_have_voice):
        ...
        # Only establish connection when there is sound and no connection (excluding stopping situation)
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        logger.bind(tag=TAG).debug(
            f"WebSocket connection established successfully, task_id: {self.task_id}"
        )
        ...
        try:
            while not conn.stop_event.is_set():
                ...
                try:
                    ...
                    if status != 20000000:
                        if status == 40010004:
                            logger.bind(tag=TAG).warning(
                                f"Please close the link after the server response is completed, status code: {status}"
                            )
                            break
                        if status in [
                            40000004,
                            40010003,
                        ]:  # Connection timeout or client disconnection
                            logger.bind(tag=TAG).warning(
                    ...
                    # Receiving TranscriptionStarted indicates the server is ready to receive audio data
                    if message_name == "TranscriptionStarted":
                        self.server_ready = True
                        logger.bind(tag=TAG).debug(
                            "Server is ready, start sending cached audio..."
                        )
                        ...
                        # Sentence end (each sentence triggers this)
                        ...
                        if text:
                            ...
                            if conn.client_listen_mode == "manual":
                                ...
                                # Manual mode only triggers processing after receiving stop signal (processed only once)
                                if conn.client_voice_stop:
                                    logger.bind(tag=TAG).debug(
                                        "Received final recognition result, triggering processing"
                                    )
                    ...
                    logger.bind(tag=TAG).error("Receiving result timeout")
                    break
                    ...
                    logger.bind(tag=TAG).error(f"Processing results failed: {str(e)}")
                    break

        except Exception as e:
            logger.bind(tag=TAG).error(f"Result forwarding failed: {str(e)}")
        finally:
            # Clean connection's audio cache
            await self._cleanup()
    ...
    async def _send_stop_request(self):
        ...
        if self.asr_ws:
            try:
                ...
                logger.bind(tag=TAG).error(
                    f"Failed to send stop recognition request: {e}"
                )

    async def _cleanup(self):
        """Clean resources (close connection)"""
        logger.bind(tag=TAG).debug(
        ...
        logger.bind(tag=TAG).debug("ASR status has been reset")
    ...
    async def close(self):
        ...
        if hasattr(self, "decoder") and self.decoder is not None:
            try:
                ...
                logger.bind(tag=TAG).debug(
                    f"Error releasing Aliyun decoder resources: {e}"
                )

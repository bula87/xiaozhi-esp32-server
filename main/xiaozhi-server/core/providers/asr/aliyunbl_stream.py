...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        self.disfluency_removal_enabled = config.get(
            "disfluency_removal_enabled", False
        )
        self.language_hints = config.get("language_hints")
        self.semantic_punctuation_enabled = config.get(
            "semantic_punctuation_enabled", False
        )
        max_sentence_silence = config.get("max_sentence_silence")
        self.max_sentence_silence = (
            int(max_sentence_silence) if max_sentence_silence else 200
        )
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        try:
            # If manual mode, set timeout to maximum
            ...
            logger.bind(tag=TAG).debug(
                f"Connecting to Aliyun Bailian ASR service, task_id: {self.task_id}"
            )
            ...
            logger.bind(tag=TAG).debug(
                "Sent run-task command, waiting for server to be ready..."
            )
            ...
            while not conn.stop_event.is_set():
                ...
                try:
                    ...
                    # Handle result-generated event
                        ...
                        # Determine if it's final result (sentence_end is True and end_time is not null)
                        ...
                        if is_final:
                            ...
                            if conn.client_listen_mode == "manual":
                                ...
                                if conn.client_voice_stop:
                                    logger.bind(tag=TAG).debug(
                                        "Received final recognition result, trigger processing"
                                    )
                                    await self.handle_voice_stop(conn, audio_data)
                                    break
                            else:
                                # Directly overwrite in automatic mode
                                self.text = text
                                await self.handle_voice_stop(conn, audio_data)
                                break
    ...
    async def _send_stop_request(self):
        """Send stop request (used to stop recording in manual mode)"""
        if self.asr_ws:
            try:
                ...
                logger.bind(tag=TAG).debug(
                    "Received stop request, send finish-task command"
                )
    ...
    async def _cleanup(self):
        ...
        if self.asr_ws:
            try:
                # First send finish-task command
                await self._send_finish_task()
    ...
    async def close(self):
        ...
        if hasattr(self, "decoder") and self.decoder is not None:
            try:
                ...
                logger.bind(tag=TAG).debug(
                    f"Error releasing Aliyun BL decoder resources: {e}"
                )

...
class ASRProviderBase(ABC):
    ...
    async def receive_audio(self, conn: "ConnectionHandler", audio, audio_have_voice):
        if conn.client_listen_mode == "manual":
            ...
            # In automatic mode, trigger recognition when VAD detects voice stop
            if (
    ...
    async def handle_voice_stop(
        ...
        try:
            ...
            # Record recognition result - check if it's an exception
            ...
            if isinstance(voiceprint_result, Exception):
                logger.bind(tag=TAG).error(
                    f"Voiceprint recognition failed: {voiceprint_result}"
                )
                speaker_name = ""
            else:
            ...
            if isinstance(raw_text, dict):
                ...
                if raw_text.get("language"):
                    logger.bind(tag=TAG).info(
                        f"Recognized language: {raw_text['language']}"
                    )
                if raw_text.get("emotion"):
                    logger.bind(tag=TAG).info(
                        f"Recognized emotion: {raw_text['emotion']}"
                    )
    ...
    def prefers_temp_file(self) -> bool:
        """Whether to prefer using temporary file"""
        return False
    ...
    def decode_opus(opus_data: List[bytes]) -> List[bytes]:
        ...
        try:
            ...
            for i, opus_packet in enumerate(opus_data):
                try:
                    ...
                    logger.bind(tag=TAG).warning(
                        f"Opus decode error, skip packet {i}: {e}"
                    )
                except Exception as e:
                    logger.bind(tag=TAG).error(
            ...
            logger.bind(tag=TAG).error(f"Audio decode process error: {e}")
            return []
        finally:

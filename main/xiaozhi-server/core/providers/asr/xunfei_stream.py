...
class ASRProvider(ASRProviderBase):
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        try:
            ...
            # If manual mode, set timeout to one minute
            ...
            while not conn.stop_event.is_set():
                try:
                    ...
                    if code != 0:
                        ...
                        if code in [10114, 10160]:  # Connection issues
                            break
                        continue
                    ...
                    logger.bind(tag=TAG).error("Result reception timeout")
                    break
    ...
    async def handle_voice_stop(
        ...
        """Process voice stop, send last frame and process recognition result"""
        try:
            # First send last frame to indicate audio end
            if self.asr_ws and self.is_processing:
                try:
                    await self._send_audio_frame(b"", STATUS_LAST_FRAME)
                    logger.bind(tag=TAG).debug("Sent stop request")
    ...
    async def close(self):
        ...
        if hasattr(self, "decoder") and self.decoder is not None:
            try:
                ...
                logger.bind(tag=TAG).debug(
                    f"Error releasing Xunfei decoder resources: {e}"
                )

...
class ASRProvider(ASRProviderBase):
    def __init__(self, config: dict, delete_audio_file: bool):
        ...
        if not self.api_key:
            raise ValueError("Qwen3-ASR-Flash requires configuring api_key")

        self.model_name = config.get("model_name", "qwen3-asr-flash")
        self.output_dir = config.get("output_dir", "./audio_output")
        self.delete_audio_file = delete_audio_file

        # ASR options configuration
        ...
        self.language = config.get(
            "language", None
        )  # Specify language, default automatic detection
        self.context = config.get(
            "context", ""
        )  # Context information, used to improve recognition accuracy
    ...
    async def speech_to_text(
        ...
        try:
            ...
            # If there is context information, add system message
            if self.context:
                messages.insert(
                    0, {"role": "system", "content": [{"text": self.context}]}
                )

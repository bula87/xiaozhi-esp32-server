...
class ASRProvider(ASRProviderBase):
    def __init__(self, config: dict, delete_audio_file: bool):
        ...
        if self.access_key_id and self.access_key_secret:
            # Use the keys to generate a temporary token
            self._refresh_token()
        else:
    ...
    def _refresh_token(self):
        ...
        if self.access_key_id and self.access_key_secret:
            ...
            if not expire_time_str:
                raise ValueError("Cannot obtain valid Token expiration time")

            try:
                # Uniformly convert to string handling
    ...
    def _is_token_expired(self):
        ...
        if not self.expire_time:
            return False  # Long-term Token not expired
        # Added debug log
    ...
    def _construct_request_url(self) -> str:
        """Construct request URL, including parameters"""

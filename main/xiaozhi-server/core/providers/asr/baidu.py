        ...
        try:
            ...
            if not self.app_id or not self.api_key or not self.secret_key:
                logger.bind(tag=TAG).error(
                    "Baidu speech recognition configuration not set, unable to perform recognition"
                )
                return None, None
            ...
            if result and result["err_no"] == 0:
                logger.bind(tag=TAG).debug(
                    f"Baidu speech recognition time-consuming: {time.time() - start_time:.3f}s | Result: {result}"
                )

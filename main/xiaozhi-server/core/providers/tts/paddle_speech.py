...
class TTSProvider(TTSProviderBase):
    ...
    def __init__(self, config, delete_audio_file):
        ...
        self.url = config.get(
            "url", "ws://192.168.1.10:8092/paddlespeech/tts/streaming"
        )
        ...
        # Apply percentage adjustment (if it exists), otherwise use public configuration
    ...
    async def text_streaming(self, text, output_file):
        try:
            ...
            async with websockets.connect(self.url) as ws:
                # Send start request
                start_request = {"task": "tts", "signal": "start"}
                ...
                if start_response.get("status") != 0:
                    raise Exception(
                        f"Connection failed: {start_response.get('signal')}"
                    )
                ...
                try:
                    while True:
                        ...
                        if status == 2:  # Last data packet
                            ...
                            audio_chunks += base64.b64decode(response_data.get("audio"))
                except asyncio.TimeoutError:
                    raise Exception(
                        f"WebSocket Timeout: Waiting for audio data for more than {timeout_seconds} seconds"
                    )
                ...
                end_request = {
                    "task": "tts",
                    "signal": "end",
                    "session": session_id,  # Session ID must be consistent with the start request
                }
                await ws.send(json.dumps(end_request))

                # Receive end response to avoid service throwing exceptions
                await ws.recv()
            ...
            raise Exception(
                f"Error during TTS WebSocket request: {e} while processing text: {text}"
            )

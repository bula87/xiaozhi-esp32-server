...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        default_pronunciation_dict = {
            "tone": ["handle/(chu3)(li3)", "danger/dangerous"]
        }
        ...
        self.voice_setting = {
            **default_voice_setting,
            **self.config.get("voice_setting", {}),
        }
        self.pronunciation_dict = {
            **default_pronunciation_dict,
            **self.config.get("pronunciation_dict", {}),
        }
        self.audio_setting = {
            **defult_audio_setting,
            **self.config.get("audio_setting", {}),
        }
        self.timber_weights = parse_string_to_list(self.config.get("timber_weights"))
        ...
        def to_tts_single_stream(self, text, is_last=False):
            try:
                ...
                if max_repeat_time > 0:
                    logger.bind(tag=TAG).info(
                        f"Speech generation successful: {original_text}, retries{5 - max_repeat_time} times"
                    )
                else:
                    logger.bind(tag=TAG).error(
                ...
                # The following lines were outside the original intended structure due to errors
                pass
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    async for chunk in resp.content.iter_any():
                        ...
                        if header_pos != -1 and end_pos != -1:
                            buffer = buffer[end_pos + 2 :]
                        else:
                            # Handle case where data extraction failed or was incomplete
                            return []

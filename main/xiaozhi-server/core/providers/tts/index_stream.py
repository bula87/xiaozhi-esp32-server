...
class TTSProvider(TTSProviderBase):
    ...
    def _process_remaining_text_stream(self, is_last=False):
        """Process remaining text and generate voice
        Returns:
    ...
    async def text_to_speak(self, text, is_last):
        """Stream process TTS audio, push only one audio list per sentence"""
    ...
    def audio_to_pcm_data_stream(self, audio_file_path, callback=None):
        """Convert audio file to PCM encoding, using 24kHz sampling rate"""
    ...
    def audio_to_opus_data_stream(self, audio_file_path, callback=None):
        """Convert audio file to Opus encoding, using 24kHz sampling rate and its own encoder"""
        from core.utils.util import audio_to_data_stream
    ...
    def to_tts(self, text: str) -> list:
        ...
        Returns:
            list: list of audio data after Opus encoding
        """
            ...
            with requests.post(self.api_url, json=payload, timeout=5) as response:
                ...
                for i in range(0, len(pcm_data), frame_bytes):
                    ...
                    if len(frame) < frame_bytes:
                        # The last frame may be insufficient, pad with 0

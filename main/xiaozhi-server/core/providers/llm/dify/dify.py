...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        self.session_conversation_map = {}  # mapping of session_id and conversation_id
        ...
        with requests.post(
            ...
            for line in r.iter_lines():
                if line.startswith(b"data: "):
                    ...
                    if self.mode == "workflows/run":
                        if event.get("event") == "workflow_finished":
                            if event["data"]["status"] == "succeeded":
                                yield event["data"]["outputs"]["answer"]
                            else:
                                yield "Service response abnormal"

                    elif self.mode == "chat-messages":
                        # Filter message_replace events, this event will be pushed entirely once
                        if event.get("event") != "message_replace" and event.get(
                            "answer"
                        ):
                            # If conversation_id is not found, retrieve it
...
def response_with_functions(self, session_id, dialogue, functions=None):
    if len(dialogue) == 2 and functions is not None and len(functions) > 0:
        # First call to llm, take the last user message, attach tool prompt
    ...
    # If the last one is role="tool", attach to user

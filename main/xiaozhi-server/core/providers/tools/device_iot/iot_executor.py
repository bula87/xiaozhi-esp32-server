...
class DeviceIoTExecutor(ToolExecutor):
    ...
    async def execute(
        ...
        try:
            # Parse tool name, get device name and operation type
            if tool_name.startswith("get_"):
                ...
                if len(parts) >= 3:
                    ...
                    if value is not None:
                        ...
                        response_failure = arguments.get(
                            "response_failure",
                            f"Unable to get the status of{device_name}",
                        )
                        return ActionResponse(
                ...
                if len(parts) >= 2:
                    ...
                    response_success = arguments.get(
                        "response_success", "Operation successful"
                    )
                ...
                return ActionResponse(
                    action=Action.ERROR, response="Cannot parse IoT tool name"
                )
                        ...
                        "function": {
                            ...
                            "parameters": {
                                ...
                                "properties": {
                                    "response_success": {
                                        "type": "string",
                                        "description": "friendly response when the query is successful, must use{value} as a placeholder to represent the queried value",
                                    },
                                    "response_failure": {
                                        "type": "string",
                                        "description": "friendly response when the query fails",
                                    },
                                },
    ...
    def get_tools(self) -> Dict[str, ToolDefinition]:
        """Get all device IoT tools"""
        return self.iot_tools.copy()

    def has_tool(self, tool_name: str) -> bool:
        """Check if there is a specified device IoT tool"""

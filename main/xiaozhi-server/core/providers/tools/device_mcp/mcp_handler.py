...
async def handle_mcp_message(
    ...
    """Handles MCP message, including initialization, tool list, and tool call responses, etc."""
    logger.bind(tag=TAG).debug(f"Handling MCP message: {str(payload)[:100]}")

    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error(
            "MCP message is missing the payload field or has a format error"
        )
        return
    ...
    if "result" in payload:
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            logger.bind(tag=TAG).debug(
                "Initialization complete, starting request for MCP tool list"
            )
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                # Handle continuation request based on the tool list response
                ...
                if next_cursor:
                    ...
                    logger.bind(tag=TAG).debug(
                        "All tools have been obtained, MCP client is ready"
                    )
...
async def send_mcp_tools_list_request(conn: "ConnectionHandler"):
    ...
    logger.bind(tag=TAG).debug("Send MCP tools list request")
    await send_mcp_message(conn, payload)
...
async def call_mcp_tool(
    ...
    try:
        ...
        # Ensure parameter is dictionary type
        if not isinstance(arguments, dict):
            raise ValueError(
                f"Parameter must be dictionary type, actual type: {type(arguments)}"
            )

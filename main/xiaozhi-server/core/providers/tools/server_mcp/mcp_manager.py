...
class ServerMCPManager:
    """Manages the centralized manager for multiple server MCP services"""
    ...
    def __init__(self, conn) -> None:
        ...
        if not os.path.exists(self.config_path):
            ...
            logger.bind(tag=TAG).warning(
                "Please check the mcp service configuration file: data/.mcp_server_settings.json"
            )
    ...
    async def _init_server(self, name: str, srv_config: Dict[str, Any]):
        ...
        try:
            ...
            logger.bind(tag=TAG).info(f"Initialize server-side MCP client: {name}")
            client = ServerMCPClient(srv_config)
            # Set timeout time to 10 seconds
            await asyncio.wait_for(
                client.initialize(logging_callback=self.logging_callback), timeout=10
            )
            ...
            logger.bind(tag=TAG).error(f"Failed to initialize MCP server {name}: {e}")
            if client:
                await client.cleanup()
    ...
    async def initialize_servers(self) -> None:
        ...
        # Output the list of server-side MCP tools currently supported
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        """Execute tool call, try to reconnect on failure"""
        logger.bind(tag=TAG).info(
            f"Execute server-side MCP tool {tool_name}, arguments: {arguments}"
        )
        ...
        for attempt in range(max_retries):
            try:
                return await target_client.call_tool(
                    tool_name, arguments, progress_callback=self.progress_callback
                )
            except Exception as e:
                # Raise exception directly when the last attempt fails
                if attempt == max_retries - 1:
                    raise
                logger.bind(tag=TAG).warning(
                    f"Executing tool {tool_name} failed (attempt {attempt + 1}/{max_retries}): {e}"
                )

                # Attempt to reconnect
                logger.bind(tag=TAG).info(
                    f"Attempt to reconnect MCP client {client_name} before retry"
                )
                try:
    ...
    async def cleanup_all(self) -> None:
        ...
        for name, client in list(self.clients.items()):
            try:
                ...
                logger.bind(tag=TAG).error(
                    f"Error closing server MCP client {name}: {e}"
                )
        self.clients.clear()

    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if len(image_data) > MAX_FILE_SIZE:
                raise ValueError(
                    f"Image size exceeds the limit, maximum allowed{MAX_FILE_SIZE / 1024 / 1024}MB"
                )
            ...
            # If the control panel is enabled, get the model configuration from the control panel
            ...
            if not select_vllm_module:
                raise ValueError(
                    "You have not yet set the default visual analysis module"
                )

            vllm_module_config = current_config["VLLM"][select_vllm_module]

            vllm_type = (
                select_vllm_module
                if "type" not in vllm_module_config
                else vllm_module_config["type"]
            )

            if not vllm_type:
                raise ValueError(
                    f"Cannot find the supplier corresponding to the VLLM module{vllm_type}"
                )

            vllm = create_instance(vllm_type, vllm_module_config)
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            if vision_explain and len(vision_explain) > 0 and "null" != vision_explain:
                message = f"MCP Vision interface is running normally, the vision explanation interface address is: {vision_explain}"
            else:

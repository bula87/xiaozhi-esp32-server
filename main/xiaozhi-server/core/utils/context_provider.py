...
class ContextDataProvider:
    """Data context filler, responsible for fetching data from configured APIs"""
                    ...
                    if isinstance(result, dict):
                        if result.get("code") == 0:
                            ...
                            self.logger.bind(tag=TAG).warning(
                                f"API {url} returned error code: {result.get('msg')}"
                            )
                    else:
                        self.logger.bind(tag=TAG).warning(
                            f"API {url} did not return a JSON dictionary"
                        )
                else:
                    self.logger.bind(tag=TAG).warning(

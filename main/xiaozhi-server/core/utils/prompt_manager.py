...
class PromptManager:
        ...
        try:
            ...
            if cached_template is not None:
                ...
                self.logger.bind(tag=TAG).debug(
                    "Loaded base prompt template from cache"
                )
                return
            ...
            if os.path.exists(template_path):
                ...
                # Store in cache (CONFIG type does not expire automatically, needs manual invalidation)
                self.cache_manager.set(
                    self.CacheType.CONFIG, cache_key, template_content
                )
                self.base_prompt_template = template_content
                self.logger.bind(tag=TAG).debug(
                    "Successfully loaded and cached base prompt template"
                )
            else:
    ...
    def get_quick_prompt(self, user_prompt: str, device_id: str = None) -> str:
        ...
        # Use provided prompt and cache it (if device ID is present)
        if device_id:
            device_cache_key = f"device_prompt:{device_id}"
            self.cache_manager.set(
    ...
    def _get_location_info(self, client_ip: str) -> str:
        ...
        try:
            ...
            # Cache miss, call API to get info
    ...
    def update_context_info(self, conn, client_ip: str):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).debug("Context information update complete")
    ...
    def build_enhanced_prompt(
        ...
        try:
            ...
            # Get selected TTS language, default is Chinese
            language = (
                self.config.get("TTS", {})

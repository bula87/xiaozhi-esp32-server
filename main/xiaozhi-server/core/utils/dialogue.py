...
class Message:
    def __init__(
        ...
        self.is_temporary = (
            is_temporary  # Mark temporary messages (e.g., tool call reminders)
        )
...
class Dialogue:
    ...
    def get_llm_dialogue(self) -> List[Dict[str, str]]:
        ...
        # This ensures speaker functionality is effective in all call paths
    ...
    def _ensure_tool_calls_complete(self, messages: List[Message]) -> List[Message]:
        ...
        Fix dangling tool_calls caused by interruptions to prevent LLM API 400 errors
        """
        ...
        for missing_id in pending_tool_calls:
            dummy_tool_msg = Message(
                role="tool",
                content='{"status": "interrupted", "message": "Action was cancelled/interrupted"}',
    ...
    def get_llm_dialogue_with_memory(
        ...
        if system_message:
            # Use <context> as a delimiter to split static system prompt and dynamic context
            # Static part (rules, identity, etc.) remains unchanged and can hit prefix cache
            # Dynamic part (time, weather, memory, etc.) is used as the second system message to maintain system authority
            ...
            # Part 1: Static system prompt (prefix cacheable)
            dialogue.append({"role": "system", "content": static_part})

        # Part 2: Few-shot examples (unchanged within the conversation, also part of prefix cache)
        ...
        # Part 3: Dynamic context system prompt (time, memory, speakers, etc.)
        # Keep system role to ensure model authority, do not downgrade to user
        if system_message and dynamic_part:
            ...
            if voiceprint_config is not None:
                dynamic_part += "\n<speakers_info>"
        ...
        # Part 4: Actual dialogue history (excluding few-shot)

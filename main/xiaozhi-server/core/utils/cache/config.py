    ...
    def for_type(cls, cache_type: CacheType) -> "CacheConfig":
        ...
        configs = {
            CacheType.LOCATION: cls(
                ...
                ttl=None,
                max_size=1000,  # Manual invalidation
            ),
            CacheType.IP_INFO: cls(
                strategy=CacheStrategy.TTL,
                ttl=86400,
                max_size=1000,  # 24 hours
            ),
            CacheType.WEATHER: cls(
                strategy=CacheStrategy.TTL,
                ttl=28800,
                max_size=1000,  # 8 hours
            ),
            CacheType.LUNAR: cls(
                strategy=CacheStrategy.TTL,
                ttl=2592000,
                max_size=365,  # 30 days expiration
            ),
            CacheType.INTENT: cls(
                strategy=CacheStrategy.TTL_LRU,
                ttl=600,
                max_size=1000,  # 10 minutes
            ),
            CacheType.CONFIG: cls(
                strategy=CacheStrategy.FIXED_SIZE,
                ttl=None,
                max_size=20,  # Manual invalidation
            ),
            ...
            CacheType.VOICEPRINT_HEALTH: cls(
                strategy=CacheStrategy.TTL,
                ttl=600,
                max_size=100,  # 10 minutes expiration
            ),

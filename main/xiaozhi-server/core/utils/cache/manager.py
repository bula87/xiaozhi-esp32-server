...
class GlobalCacheManager:
    ...
    @property
    def logger(self):
        """Lazily initialize logger to avoid circular imports"""
    ...
    def set(
        ...
        with self._locks[cache_name]:
            ...
            if config.strategy in [CacheStrategy.LRU, CacheStrategy.TTL_LRU]:
                # LRU strategy: if already exists, move to end
                if key in cache:
                    del cache[key]
                cache[key] = entry
    ...
    def _maybe_cleanup(self, cache_name: str):
        ...
        if now - self._last_cleanup > config.cleanup_interval:
            ...
            if deleted > 0:
                ...
                self.logger.debug(
                    f"Cleaned cache {cache_name}: deleted {deleted} expired entries"
                )

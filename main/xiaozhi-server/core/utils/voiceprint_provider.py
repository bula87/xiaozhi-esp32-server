    ...
    def __init__(self, config: dict):
        ...
        if not self.original_url:
            logger.bind(tag=TAG).warning(
                "Voiceprint recognition URL not configured, voiceprint recognition will be disabled"
            )
            self.enabled = False
        else:
            # Parse URL and key
            ...
            if not self.api_key:
                logger.bind(tag=TAG).error(
                    "No key parameter found in URL, voiceprint recognition will be disabled"
                )
                self.enabled = False
            else:
                # Construct identify API address
                ...
                if not self.speaker_ids:
                    logger.bind(tag=TAG).warning(
                        "No valid speakers configured, voiceprint recognition will be disabled"
                    )
                    self.enabled = False
                else:
                    # Perform health check to verify if server is available
                    if self._check_server_health():
                        self.enabled = True
                        logger.bind(tag=TAG).info(
                            f"Voiceprint recognition enabled: API={self.api_url}, speakers={len(self.speaker_ids)}, similarity threshold={self.similarity_threshold}"
                        )
                    else:
                        self.enabled = False
                        logger.bind(tag=TAG).warning(
                            f"Voiceprint recognition server unavailable, voiceprint recognition disabled: {self.api_url}"
                        )
    ...
    def _parse_speakers(self) -> Dict[str, Dict[str, str]]:
        ...
        for speaker_str in self.speakers:
            try:
                ...
                logger.bind(tag=TAG).warning(
                    f"Failed to parse speaker configuration: {speaker_str}, error: {e}"
                )
        return speaker_map
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("status") == "healthy":
                    logger.bind(tag=TAG).info(
                        "Voiceprint recognition server health check passed"
                    )
                    is_healthy = True
                else:
                    logger.bind(tag=TAG).warning(
                        f"Voiceprint recognition server abnormal status: {result}"
                    )
                    is_healthy = False
            else:
                logger.bind(tag=TAG).warning(
                    f"Voiceprint recognition server health check failed: HTTP {response.status_code}"
                )
                is_healthy = False

        except requests.exceptions.ConnectTimeout:
            logger.bind(tag=TAG).warning(
                "Voiceprint recognition server connection timed out"
            )
            is_healthy = False
        except requests.exceptions.ConnectionError:
            logger.bind(tag=TAG).warning(
                "Voiceprint recognition server connection refused"
            )
            is_healthy = False
        except Exception as e:
            logger.bind(tag=TAG).warning(
                f"Voiceprint recognition server health check exception: {e}"
            )
            is_healthy = False
    ...
    async def identify_speaker(
        ...
        if not self.enabled or not self.api_url or not self.api_key:
            logger.bind(tag=TAG).debug(
                "Voiceprint recognition function is disabled or not configured, skipping identification"
            )
            return None
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.bind(tag=TAG).info(
                            f"Voiceprint recognition time: {total_elapsed_time:.3f}s"
                        )

                        # Similarity threshold check
                        if score < self.similarity_threshold:
                            logger.bind(tag=TAG).warning(
                                f"Voiceprint recognition similarity {score:.3f} is below threshold {self.similarity_threshold}"
                            )
                        ...
                        if speaker_id and speaker_id in self.speaker_map:
                            ...
                            logger.bind(tag=TAG).info(
                                f"Voiceprint recognition succeeded: {result_name} (similarity: {score:.3f})"
                            )
                            return result_name
                        else:
                            logger.bind(tag=TAG).warning(
                                f"Unrecognized speaker ID: {speaker_id}"
                            )
                            return "Unknown speaker"
                    else:
                        logger.bind(tag=TAG).error(
                            f"Voiceprint recognition API error: HTTP {response.status}"
                        )
                        return None
            ...
            logger.bind(tag=TAG).error(
                f"Voiceprint recognition timeout: {elapsed:.3f}s"
            )
            return None
        except Exception as e:
            elapsed = time.monotonic() - api_start_time

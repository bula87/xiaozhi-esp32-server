...
def initialize_modules(
    ...
    if init_tts:
        ...
        logger.bind(tag=TAG).info(
            f"Component initialized: tts success {select_tts_module}"
        )
    ...
    if init_llm:
        ...
        logger.bind(tag=TAG).info(
            f"Component initialized: llm success {select_llm_module}"
        )
    ...
    if init_intent:
        ...
        logger.bind(tag=TAG).info(
            f"Component initialized: intent success {select_intent_module}"
        )
    ...
    if init_memory:
        ...
        logger.bind(tag=TAG).info(
            f"Component initialized: memory success {select_memory_module}"
        )
    ...
    if init_vad:
        ...
        logger.bind(tag=TAG).info(
            f"Component initialized: vad success {select_vad_module}"
        )
    ...
    if init_asr:
        ...
        logger.bind(tag=TAG).info(
            f"Component initialized: asr success {select_asr_module}"
        )
    return modules
...
def initialize_voiceprint(asr_instance, config):
    ...
    if not voiceprint_config.get("url") or not voiceprint_config.get("speakers"):
        logger.bind(tag=TAG).warning(
            "Voiceprint recognition configuration is incomplete"
        )
        return False
    ...
    try:
        ...
        logger.bind(tag=TAG).info(
            "ASR module voiceprint recognition feature dynamically enabled"
        )
        logger.bind(tag=TAG).info(
            f"Configured number of speakers: {len(voiceprint_config['speakers'])}"
        )
        return True
    except Exception as e:
        logger.bind(tag=TAG).error(
            f"Failed to dynamically initialize voiceprint recognition feature: {str(e)}"
        )
        return False

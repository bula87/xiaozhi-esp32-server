...
Periodically performs garbage collection to avoid frequent GC triggering GIL lock issues
"""
...
class GlobalGCManager:
    ...
    async def start(self):
        ...
        logger.bind(tag=TAG).info(
            f"Starting global GC manager, interval {self.interval_seconds} seconds"
        )
    ...
    async def _run_gc(self):
        ...
        try:
            # Run GC in thread pool to avoid blocking event loop
            ...
            logger.bind(tag=TAG).error(f"Error occurred during GC execution: {e}")

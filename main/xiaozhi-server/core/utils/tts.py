...
def create_instance(class_name, *args, **kwargs):
    ...
    raise ValueError(
        f"Unsupported TTS type: {class_name}. Please check if the 'type' in the configuration is set correctly."
    )


class MarkdownCleaner:
    """
    Encapsulate Markdown cleaning logic: just use MarkdownCleaner.clean_markdown(text)
    """
    ...
    def _replace_table_block(match: re.Match) -> str:
        """
        Callback when a whole table block is matched.
        """
        ...
        if len(parsed_table) == 1:
            ...
            lines_for_tts.append(f"Table header: {', '.join(headers)}")
    ...
    # Precompile all regular expressions (ordered by frequency of use)
    # The replace_xxx static methods must be defined first so they can be referenced here.
    REGEXES = [
    ...
    def clean_markdown(text: str) -> str:
        """
        Main entry method: sequentially apply all regexes to remove or replace Markdown elements
        """
...
def convert_percentage_to_range(percentage, min_val, max_val, base_val=None):
    ...
    if percentage < 0:
        # Negative percentage: interpolate linearly from base_val to min_val
        result = base_val + (base_val - min_val) * (percentage / 100)
    else:
        # Positive percentage: interpolate linearly from base_val to max_val

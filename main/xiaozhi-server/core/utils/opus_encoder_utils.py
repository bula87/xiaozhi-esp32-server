...
class OpusEncoderUtils:
    ...
    def __init__(self, sample_rate: int, channels: int, frame_size_ms: int):
        ...
        try:
            ...
            self.encoder = Encoder(
                sample_rate,
                channels,
                constants.APPLICATION_AUDIO,  # audio optimized mode
            )
            self.encoder.bitrate = self.bitrate
            self.encoder.complexity = self.complexity
            self.encoder.signal = constants.SIGNAL_VOICE  # voice signal optimized
    ...
    def encode_pcm_to_opus_stream(
        ...
        """
        Encode PCM data to Opus format, process in streaming fashion
    ...
    def _validate_pcm_data(self, pcm_shorts: np.ndarray) -> None:
        ...
        if np.any((pcm_shorts < -32768) | (pcm_shorts > 32767)):
            ...
            # In actual applications, you may choose to clip instead of raising an exception

    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        if headers.get("device-id", None) is None:
            ...
            if "device-id" not in query_params:
                await websocket.send(
                    "Port is normal. To test the connection, please use test_page.html"
                )
                await websocket.close()
                return
            else:
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(
                f"Error occurred while handling connection: {e}"
            )
        finally:
            ...
            try:
                ...
                self.logger.bind(tag=TAG).error(
                    f"Error occurred while forcibly closing connection on server: {close_error}"
                )
    ...
    async def update_config(self) -> bool:
        ...
        try:
            async with self.config_lock:
                ...
                self.logger.bind(tag=TAG).info("Config update task completed")
                return True
        except Exception as e:
    ...
    async def _handle_auth(self, websocket: websockets.ServerConnection):
        ...
        if self.auth_enable:
            ...
            if self.allowed_devices and device_id in self.allowed_devices:
                # If device is in whitelist, skip token validation
                return
            else:

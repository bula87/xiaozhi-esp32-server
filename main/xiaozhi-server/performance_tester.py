...
print(
    "Before use, please prepare the configuration according to the instructions in doc/performance_testerer.md."
)
...
def main():
    ...
    if not modules:
        print(
            "No available performance testing tools in the performance_tester directory."
        )
        return
    ...
    try:
        choice = (
            int(
                input(
                    "Please select the number of the performance testing tool to use: "
                )
            )
            - 1
        )

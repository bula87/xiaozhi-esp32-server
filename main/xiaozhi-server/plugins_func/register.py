...
class ToolType(Enum):
    ...
    WAIT = (2, "Call the tool and wait for function return")
    CHANGE_SYS_PROMPT = (
        3,
        "Change system prompt, switch role personality or responsibility",
    )
    SYSTEM_CTL = (
        4,
        "System control, affects normal dialogue flow (e.g., exit, play music), requires passing conn parameter",
    )
...
class Action(Enum):
    ...
    REQLLM = (3, "Call function then request LLM to generate reply")
    RECORD = (4, "Record tool call to dialogue history, do not call LLM")
...
def register_function(name, desc, type=None):
    """Decorator to register function to the function registry dictionary"""

    def decorator(func):
        all_function_registry[name] = FunctionItem(name, desc, func, type)
        logger.bind(tag=TAG).debug(
            f"Function '{name}' loaded and can be registered for use"
        )
        return func
...
def register_device_function(name, desc, type=None):
    """Decorator to register device-level function to the function registry dictionary"""
...
class FunctionRegistry:
    ...
    def unregister_function(self, name):
        # Unregister function, check if it exists

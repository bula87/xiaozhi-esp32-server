...
play_music_function_desc = {
    ...
    "function": {
        "name": "play_music",
        "description": "Method for singing, listening to, and playing music.",
        "parameters": {
            "type": "object",
            "properties": {
                "song_name": {
                    "type": "string",
                    "description": "Song name. If the user does not specify a song, use 'random'. If specified, return the music name. Example: ```User: Play Two Tigers\nParameter: Two Tigers``` ```User: Play music \nParameter: random ```",
                }
            },
            "required": ["song_name"],
        },
...
def play_music(conn: "ConnectionHandler", song_name: str):
    try:
        music_intent = (
            f"Play music {song_name}"
            if song_name != "random"
            else "Play music randomly"
        )

        # Check event loop status
        if not conn.loop.is_running():
            conn.logger.bind(tag=TAG).error(
                "Event loop not running, cannot submit task"
            )
        ...
        # Non-blocking callback handler
        def handle_done(f):
            try:
                f.result()  # Handle success logic here
                conn.logger.bind(tag=TAG).info("Playback finished")
        ...
        return ActionResponse(
            action=Action.RESPONSE,
            result=str(e),
            response="An error occurred while playing music",
        )
    ...
    if MUSIC_CACHE == {}:
        ...
        if "play_music" in plugins_config:
            ...
            MUSIC_CACHE["music_dir"] = os.path.abspath(
                MUSIC_CACHE["music_config"].get("music_dir", "./music")  # Default path
            )
    ...
    if os.path.exists(MUSIC_CACHE["music_dir"]):
        ...
        if potential_song:
            ...
            if best_match:
                ...
                return True
    # Check if it is a general play music command
...
def _get_random_play_prompt(song_name):
    ...
    # Use random.choice directly, do not set seed

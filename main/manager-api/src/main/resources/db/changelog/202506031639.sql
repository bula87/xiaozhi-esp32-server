...
-- Update Doubao ASR Configuration Explanation
UPDATE `ai_model_config` SET 
`doc_link` = 'https://console.volcengine.com/speech/app',
`remark` = 'Doubao ASR Configuration Explanation:
1. The difference between Doubao ASR and Doubao (Streaming) ASR is: Doubao ASR is charged per request, whereas Doubao (Streaming) ASR is charged per time.
2. Generally, pay-per-request is cheaper, but Doubao (Streaming) ASR uses large model technology, yielding better results.

...
public class SysParamsServiceImpl extends BaseServiceImpl<SysParamsDao, SysParamsEntity> implements SysParamsService {
    ...
    private void initSM2KeyPair() {
        ...
        // If the public or private key is empty, generate a new key pair
    ...
    /**
     * Detect if SMS parameters meet the requirements
     * 
    ...
    private boolean detectingSMSParameters(String paramCode, String paramValue) {
        // Check if it is the parameter code for enabling mobile registration, if not, no need to check other SMS parameters, return true directly
        if (!Constant.SysMSMParam.SERVER_ENABLE_MOBILE_REGISTER.getValue().equals(paramCode)) {
            return true;
        }
        // Check if it is disabled, if disabled, no need to check other SMS parameters, return true directly
        if ("false".equalsIgnoreCase(paramValue)) {
            return true;
        }
        // Check if the associated SMS parameters are empty

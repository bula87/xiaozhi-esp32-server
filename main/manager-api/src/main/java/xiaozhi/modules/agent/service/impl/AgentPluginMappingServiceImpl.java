...
/**
 * @description For the database operation Service implementation of table 【ai_agent_plugin_mapping(Agent and plugin unique mapping table)】
    ...
    public List<AgentPluginMapping> agentPluginParamsByAgentId(String agentId) {
        ...
        if (knowledgeBaseMap.size() > 0) {
            for (String pluginCode : knowledgeBaseMap.keySet()) {
                ...
                String description = "If the user asks about content related to the scope covered by 【"
                        + String.join(",", knowledgeBaseList.stream().map(KnowledgeBaseEntity::getName).toList())
                        + "】, this method should be called for querying: " + String.join(",",

...
/**
 * @author chenerlei
 * @description Database operation Service implementation for the table 【ai_agent_template(智能体配置模板表)】
...
public class AgentTemplateServiceImpl extends ServiceImpl<AgentTemplateDao, AgentTemplateEntity>
    ...
    public void reorderTemplatesAfterDelete(Integer deletedSort) {
        ...
        // Query all records with sort values greater than the deleted template
        UpdateWrapper<AgentTemplateEntity> updateWrapper = new UpdateWrapper<>();
        updateWrapper.gt("sort", deletedSort)
                    .setSql("sort = sort - 1");
        
        // Execute batch update, decrementing the sort values of these records by 1
    ...
    public Integer getNextAvailableSort() {
        ...
        for (Integer sort : sortValues) {
            if (sort > expectedSort) {
                // Found a missing sequence number
                return expectedSort;
            }
            expectedSort = sort + 1;
        }
        
        // If no gaps exist, return the maximum sequence number plus 1
        return expectedSort;
    }
}

...
public class OTAController {
    ...
    public ResponseEntity<String> checkOTAVersion(
        ...
        // Device ID and MAC address should be consistent, and the application field is required
        if (!macAddressValid) {
    ...
    public ResponseEntity<String> getOTA() {
        ...
        if (StringUtils.isBlank(mqttUdpConfig)) {
            return ResponseEntity.ok("OTA interface is abnormal，missing mqtt_gateway address，please log in to the intelligent control platform，find the 【server.mqtt_gateway】 configuration in parameter management");
        }
        String wsUrl = sysParamsService.getValue(Constant.SERVER_WEBSOCKET, true);
        if (StringUtils.isBlank(wsUrl) || wsUrl.equals("null")) {
            return ResponseEntity.ok("OTA interface is abnormal，missing websocket address，please log in to the intelligent control platform，find the 【server.websocket】 configuration in parameter management");
        }
        String otaUrl = sysParamsService.getValue(Constant.SERVER_OTA, true);
        if (StringUtils.isBlank(otaUrl) || otaUrl.equals("null")) {
            return ResponseEntity.ok("OTA interface is abnormal，missing ota address，please log in to the intelligent control platform，find the 【server.ota】 configuration in parameter management");
        }
        return ResponseEntity.ok("OTA interface runs normally，websocket cluster count：" + wsUrl.split(";").length);
    }
    ...
    /**
     * Simple check if MAC address is valid (non-strict)
     * 
     * @param macAddress
     * @return
     */
    private boolean isMacAddressValid(String macAddress) {
        ...
        // MAC address is usually 12-digit hexadecimal number，can contain colon or hyphen separators
